...
public final class XpDiagnosticManager extends CarEcuManager {
    ...
    public XpDiagnosticManager(Car car, IBinder service, IBinder vehicleService, Context context, Handler handler) {
        ...
        this.mCarPropertyMgr = new CarPropertyManager(car, ICarProperty.Stub.asInterface(service), false, TAG);
            ...
            for (Integer id : ids) {
                ...
                if (config != null && isPropertyAllowRegister(config)) {
                    ...
                    if (shouldRegister) {
                        try {
                            try {
                                this.mService.registerListener(id.intValue(), CarPropertyManager.SENSOR_RATE_ONCHANGE, this.mListenerToBase);
