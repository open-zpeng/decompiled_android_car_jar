...
public final class CarDiagnosticEvent implements Parcelable {
    ...
    public void writeToJson(JsonWriter jsonWriter) throws IOException {
        ...
        for (int i2 = 0; i2 < this.intValues.size(); i2++) {
            jsonWriter.beginObject();
            jsonWriter.name("id").value(this.intValues.keyAt(i2));
            jsonWriter.name("value").value(this.intValues.valueAt(i2));
            jsonWriter.endObject();
        }
        jsonWriter.endArray();
        jsonWriter.name("floatValues").beginArray();
        for (int i3 = 0; i3 < this.floatValues.size(); i3++) {
            jsonWriter.beginObject();
            jsonWriter.name("id").value(this.floatValues.keyAt(i3));
            jsonWriter.name("value").value(this.floatValues.valueAt(i3));
            jsonWriter.endObject();
        }
        jsonWriter.endArray();
        if (this.dtc != null) {
    ...
    public boolean equals(Object otherObject) {
        ...
        for (int i2 = 0; i2 < this.floatValues.size(); i2++) {
            ...
            if (Float.compare(value2, otherValue2) != 0) {
                return false;
            }
        }
        return true;
    ...
    public CommonIgnitionMonitors getIgnitionMonitors() {
        ...
        int intValue = ignitionMonitorsType.intValue();
        if (intValue != 0) {
            if (intValue != 1) {
                return null;
            }
