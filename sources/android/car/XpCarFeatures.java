...
public class XpCarFeatures {
    ...
    public static String getPartNumber() {
        return SystemProperties.get(PART_NUMBER_CFC_CODE, CarBluetoothManager.BLUETOOTH_NO_PRIORITY_DEVICE);
    }
    ...
    public static String getVinCode() {
        return SystemProperties.get(VIN_CODE_CFC, CarBluetoothManager.BLUETOOTH_NO_PRIORITY_DEVICE);
    }

    public static String getMcuSoftwareVersion() {
        return SystemProperties.get(MCU_VERSION_CFC_CODE, CarBluetoothManager.BLUETOOTH_NO_PRIORITY_DEVICE);
    }

    public static String getMcuHardwareId() {
        return SystemProperties.get(MCU_HARDWAREID_CFC_CODE, CarBluetoothManager.BLUETOOTH_NO_PRIORITY_DEVICE);
    }
    ...
    public static int hasPM2_5Feature() {
        return getCfcIntVal(PM25_CFC_CODE, 1);
    }
    ...
    private static int getCfcIntVal(String cfcCode, int defaultVal) {
        ...
        if (!Build.IS_USER && SystemProperties.getInt(CARFEATURE_DEBUG_CFC_FLAG, 0) == 1) {
            ret = SystemProperties.get(cfcCode + ".debug", CarBluetoothManager.BLUETOOTH_NO_PRIORITY_DEVICE);
            if (!TextUtils.isEmpty(ret)) {
                Log.i(TAG, " get debug cfc config:" + cfcCode + ".debug value : " + ret);
            }
        }
        if (TextUtils.isEmpty(ret)) {
            ret = SystemProperties.get(cfcCode, CarBluetoothManager.BLUETOOTH_NO_PRIORITY_DEVICE);
        }
        if (!TextUtils.isEmpty(ret)) {
            try {
