...
public abstract class CarEcuManager extends CarManagerBase {
    ...
    protected void handleOnErrorEvent(int propertyId, int errorCode) {
        ...
        if (callbacks == null) {
            String str2 = this.mTag;
            Log.w(str2, "handleOnChangeEvent no callbacks for " + XpDebugLog.getPropertyDescription(propertyId) + " errorCode:" + errorCode);
            return;
        }
        Iterator<CarEcuEventCallback> it = callbacks.iterator();
        while (it.hasNext()) {
    ...
    public CarEcuManager(Car car, IBinder service, boolean dbg, String tag) {
        super(car);
        ...
        this.mTag = tag;
        this.mCarPropertyMgr = new CarPropertyManager(car, ICarProperty.Stub.asInterface(service), dbg, tag);
        ...
        kPackageName.computeIfAbsent("pkgName", new Function() { // from class: android.car.hardware.-$$Lambda$CarEcuManager$UaOtEfe-IVwWrsxbYW3LSvSbio4
            @Override // java.util.function.Function
            public final Object apply(Object obj) {
                return CarEcuManager.this.lambda$new$1$CarEcuManager((String) obj);
            }
        });
    }

    public /* synthetic */ Boolean lambda$new$1$CarEcuManager(String k) {
        Context context = getContext();
        return Boolean.valueOf(context != null && Objects.equals(AUTOPILOT_PACKAGE_NAME, context.getPackageName()));
    }

    public CarEcuManager(Car car, IBinder propertyService, IBinder sharedMemoryService, boolean dbg, String tag) {
        super(car);
        this.mListenerToBase = null;
        this.mLock = new Object();
        this.mSupportedPropConfigMaps = null;
        ...
        this.mIdToCallbackCountMap = new ConcurrentHashMap();
        this.mTag = tag;
        this.mCarPropertyMgr = new CarPropertyManager(car, ICarProperty.Stub.asInterface(propertyService), dbg, tag);
        this.mCarSharedMemoryMgr = new CarSharedMemoryManager(car, sharedMemoryService, dbg, tag);
        ...
        kPackageName.computeIfAbsent("pkgName", new Function() { // from class: android.car.hardware.-$$Lambda$CarEcuManager$8OafObwewPvpkdmPiuNz2wOm_ow
            @Override // java.util.function.Function
            public final Object apply(Object obj) {
                return CarEcuManager.this.lambda$new$2$CarEcuManager((String) obj);
            }
        });
    }

    public /* synthetic */ Boolean lambda$new$2$CarEcuManager(String k) {
        Context context = getContext();
        return Boolean.valueOf(context != null && Objects.equals(AUTOPILOT_PACKAGE_NAME, context.getPackageName()));
    }
    ...
    public void registerCallback(CarEcuEventCallback callback) throws CarNotConnectedException {
        Slog.e(this.mTag, "registerPropCallback(callback) is not recommended ", new RuntimeException("car-sdk"));
        registerPropCallback(getReadSupportedPropIds(), callback);
        CarSharedMemoryManager carSharedMemoryManager = this.mCarSharedMemoryMgr;
        if (carSharedMemoryManager != null) {
            carSharedMemoryManager.registerPropCallback(getSharedMemoryIds(), callback);
        }
    }
    ...
    public void unregisterCallback(CarEcuEventCallback callback) throws CarNotConnectedException {
        ...
        CarSharedMemoryManager carSharedMemoryManager = this.mCarSharedMemoryMgr;
        if (carSharedMemoryManager != null) {
            carSharedMemoryManager.unregisterCallback(callback);
        }
    }
    ...
    public void sendDataWithSharedMemory(int prop, byte[] data) throws Exception {
        CarSharedMemoryManager carSharedMemoryManager = this.mCarSharedMemoryMgr;
        if (carSharedMemoryManager != null) {
            carSharedMemoryManager.sendData(prop, data);
        }
    }
    ...
    /* JADX WARN: Code restructure failed: missing block: B:44:0x012b, code lost:
        r6 = r12.mTag;
        android.util.Slog.w(r6, "register prop: " + r4 + "(0x" + java.lang.Integer.toHexString(r4.intValue()) + ") is not allowed");
     */
    ...
    /* JADX WARN: Code restructure failed: missing block: B:61:0x020b, code lost:
        r8 = r12.mCarPropertyMgr.registerCallback(r7, r4.intValue(), -r15);
     */
    ...
    /* JADX WARN: Code restructure failed: missing block: B:65:0x0224, code lost:
        r9 = r12.mTag;
        android.util.Slog.w(r9, "call registerCallback failed for prop: " + android.car.XpDebugLog.getPropertyDescription(r4.intValue()));
     */
    ...
    /* JADX WARN: Code restructure failed: missing block: B:70:0x024f, code lost:
        r8 = r12.mTag;
        android.util.Slog.w(r8, "call registerCallback failed for prop: " + android.car.XpDebugLog.getPropertyDescription(r4.intValue()));
     */
    ...
    /* JADX WARN: Code restructure failed: missing block: B:74:0x0278, code lost:
        r8 = r12.mTag;
        android.util.Slog.w(r8, "listenerToBase is null for prop " + android.car.XpDebugLog.getPropertyDescription(r4.intValue()));
     */
    ...
    /* JADX WARN: Code restructure failed: missing block: B:78:0x02c0, code lost:
        r8 = r12.mTag;
        android.util.Slog.w(r8, "won't register the registered prop " + android.car.XpDebugLog.getPropertyDescription(r4.intValue()));
     */
    ...
    */
    ...
    private boolean tryHandleAutoPilotAppRegisterFailed(Collection<Integer> ids, int id) {
        ...
        if (result) {
            String str = this.mTag;
            Slog.w(str, "AutoPilot app register: " + idsToString(ids));
        }
        return result;
    }
    ...
    public void unregisterPropCallback(Collection<Integer> ids, CarEcuEventCallback callback) throws CarNotConnectedException {
        CarSharedMemoryManager carSharedMemoryManager;
        if (ids == null) {
                        ...
                        if (callbacks == null) {
                            ...
                            if (unregister) {
                                if (listenerToBase != null) {
                                    ...
                                    this.mCarPropertyMgr.unregisterCallback(listenerToBase, id.intValue());
                                } else {
                                    String str5 = this.mTag;
            ...
            if (sharedMemoryIds != null && (carSharedMemoryManager = this.mCarSharedMemoryMgr) != null) {
                carSharedMemoryManager.unregisterPropCallback(sharedMemoryIds, callback);
            }
        }
    }
        ...
        if (!this.mIsPropConfigsFetched) {
            synchronized (this.mLock) {
                if (!this.mIsPropConfigsFetched) {
                    ...
                    if (!this.mIsPropConfigsFetched) {
                        String str = this.mTag;
                        Slog.w(str, "SupportedPropConfigs is " + configsToString(this.mSupportedPropConfigs));
                    }
                }
            }
        }
