...
public final class CarCabinManager implements CarManagerBase {
    ...
    public synchronized void registerCallback(CarCabinEventCallback callback) throws CarNotConnectedException {
        ...
        for (CarPropertyConfig c : configs) {
            this.mCarPropertyMgr.registerListener(this.mListenerToBase, c.getPropertyId(), 0.0f);
        }
        this.mCallbacks.add(callback);
    }

    public synchronized void unregisterCallback(CarCabinEventCallback callback) throws CarNotConnectedException {
        this.mCallbacks.remove(callback);
        List<CarPropertyConfig> configs = getPropertyList();
        for (CarPropertyConfig c : configs) {
            this.mCarPropertyMgr.unregisterListener(this.mListenerToBase, c.getPropertyId());
        }
        if (this.mCallbacks.isEmpty()) {
