...
public final class VmsOperationRecorder {
    ...
    private void recordOp(String operation, String intArgName, int arg, VmsLayer layer) {
        if (isEnabled()) {
            try {
                recordOp(operation, new JSONObject().put(intArgName, arg).put("layer", toJson(layer)));
            } catch (JSONException e) {
                Log.e(TAG, e.toString());
            }
        }
    ...
    private static JSONObject toJson(VmsLayer layer) throws JSONException {
        return new JSONObject().put("type", layer.getType()).put("subtype", layer.getSubtype()).put("version", layer.getVersion());
    }
    ...
    private void write(JSONObject object) {
        this.mWriter.write(object.toString());
    }
